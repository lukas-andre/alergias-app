/**
 * Risk Evaluation V2 - Explainable & Evidence-Based
 *
 * Enhanced risk assessment that:
 * - Uses structured mentions from Vision API V2
...
import type { IngredientsResultV2, Mention, DetectedAllergen } from "@/lib/openai/vision-v2-types";
import type {
  ProfilePayload,
  RiskAssessmentV2,
  RiskReasonV2,
 ...
 *
 * When a user has a diet, these ingredients are blocked
 */
const DIET_BLOCKS: Record<string, string[]> = {
  ...
  halal: [], // Handled separately (pork, alcohol)
  kosher: [], // Handled separately
};

/**
 * Intolerance triggers
 *
 * FODMAP: high-fermentable carbs
...
/**
 * Main risk evaluation function (V2)
 *
 * Uses mentions from Vision API V2 to provide explainable,
...
export function evaluateRiskV2(
  ...
  const actions: RiskAssessmentV2["actions"] = ["guardar"];
  if (level === "high") {
